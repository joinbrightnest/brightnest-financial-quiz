...
const SECTION_TO_ROUTE: Record<SectionType, string> = {
...
const ROUTE_TO_SECTION: Record<string, SectionType> = {
...
export default function Sidebar({
    ...
    // Determine active section from URL if available, otherwise use prop
    const currentSection = ROUTE_TO_SECTION[pathname] || activeSection;

    // Handle navigation - use router.push for URL changes
    const handleNavigation = (section: SectionType) => {
        const route = SECTION_TO_ROUTE[section];
        if (route) {
            router.push(route);
        }
        // Also call onSectionChange for backwards compatibility
    ...
    const getButtonClasses = (section: SectionType) => {
        ...
        return `w-full flex items-center ${collapsed ? 'justify-center' : 'space-x-2.5'} ${collapsed ? 'px-2 py-2' : 'px-2.5 py-2 rounded-lg'
    ...
    const getIconClasses = (section: SectionType) => {
        return `w-4 h-4 ${activeSection === section ? 'text-white' : 'text-slate-300'}`;
    };
    ...
    return (
        <>
            ...
            <button
                ...
                className="md:hidden fixed top-4 left-4 z-40 p-2 bg-slate-800 text-white rounded-lg shadow-lg hover:bg-slate-700 transition-colors"
            ...
            {/* Sidebar */}
            <div className={`${collapsed ? 'w-14' : 'w-56'} bg-gradient-to-b from-slate-800 to-slate-900 border-r border-slate-700 flex flex-col transition-all duration-300 ease-in-out
                ${mobileMenuOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0
                fixed md:static inset-y-0 left-0 z-50 md:z-auto`}>
                {/* Header */}
                <div className="flex-shrink-0 border-b border-slate-700">
                    <div className={`${collapsed ? 'p-3' : 'p-4'} flex items-center justify-between`}>
                        <div className="flex items-center space-x-2.5">
                            <div className={`w-7 h-7 bg-gradient-to-br from-indigo-600 to-purple-600 ${collapsed ? 'rounded' : 'rounded-lg'} flex items-center justify-center shadow-md`}>
                                <span className="text-white font-bold text-xs">B</span>
                            </div>
                            {!collapsed && (
                                <div>
                                    <h1 className="text-base font-bold text-white">BrightNest</h1>
                                    <p className="text-[10px] text-slate-300">Admin Panel</p>
                                </div>
                            )}
                        </div>
                    </div>
                ...
                <div className={`flex-1 overflow-y-auto overflow-x-hidden ${collapsed ? 'p-2' : 'p-3'}`}>
                    <nav className="space-y-2">
                        ...
                        <button onClick={() => handleNavigation('quiz-analytics')} className={getButtonClasses('quiz-analytics')}>
                        ...
                        <button onClick={() => handleNavigation('crm')} className={getButtonClasses('crm')}>
                        ...
                        <button onClick={() => handleNavigation('ceo-analytics')} className={getButtonClasses('ceo-analytics')}>
                    ...
                    <div className="mt-6 pt-4 border-t border-slate-700">
                        ...
                        <button
                            ...
                            className={`w-full flex items-center ${collapsed ? 'justify-center' : 'space-x-2.5'} ${collapsed ? 'px-2 py-2' : 'px-2.5 py-2 rounded-lg'} text-slate-300 hover:bg-slate-700 transition-colors group relative`}
                        >
                        ...
                        <button onClick={() => handleNavigation('closer-management')} className={getButtonClasses('closer-management')}>
                        ...
                        <button onClick={() => handleNavigation('settings')} className={getButtonClasses('settings')}>
                ...
                <div className={`flex-shrink-0 border-t border-slate-700 ${collapsed ? 'p-2' : 'p-3'}`}>
                    <button
