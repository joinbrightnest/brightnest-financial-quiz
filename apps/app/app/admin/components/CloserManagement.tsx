      ...
      {isLoading ? (
        <div className="py-6 animate-pulse">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {[...Array(6)].map((_, i) => (
              <div key={i} className="bg-white border border-slate-200 rounded-xl p-6 h-48 flex flex-col justify-between shadow-sm">
                          ...
                          <div className="flex items-start space-x-4">
                            ...
                            <div>
                              ...
                              <div className="flex items-center gap-2 mt-2">
                                <span className={`inline-flex px-3 py-1 text-xs font-semibold rounded-full ${closer.isApproved ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'
                                  }`}>
                                  {closer.isApproved ? 'Approved' : 'Pending'}
                                </span>
                                <span className={`inline-flex px-3 py-1 text-xs font-semibold rounded-full ${closer.isActive ? 'bg-blue-100 text-blue-700' : 'bg-red-100 text-red-700'
                                  }`}>
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-3xl font-bold text-green-900 mt-1">
                        {appointments.filter(a => a.outcome === 'converted' && a.saleValue !== null && a.saleValue !== undefined && Number(a.saleValue) > 0).length}
                      </p>
                    </div>
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-3xl font-bold text-blue-900 mt-1">
                        ${appointments.filter(a => a.outcome === 'converted' && a.saleValue !== null && a.saleValue !== undefined && Number(a.saleValue) > 0).reduce((sum, a) => sum + (Number(a.saleValue) || 0), 0).toFixed(2)}
                      </p>
                    </div>
                          ...
                          return (
                            <div key={key} className="border border-gray-200 rounded-lg">
                              ...
                              >
                                ...
                                <svg
                                  className={`w-5 h-5 text-gray-500 transition-transform ${expandedProgramSection === key ? 'rotate-180' : ''
                                    }`}
                                  fill="none"
                                  stroke="currentColor"
                                  viewBox="0 0 24 24"
                        ...
                        {Object.entries(scriptForm.emailTemplates || {}).map(([key, template]: [string, EmailTemplate]) => (
                          <div key={key} className="border border-gray-200 rounded-lg">
                            ...
                            >
                              ...
                              <svg
                                className={`w-5 h-5 text-gray-500 transition-transform ${expandedEmailSection === key ? 'rotate-180' : ''
                                  }`}
                                fill="none"
                                stroke="currentColor"
                                viewBox="0 0 24 24"
