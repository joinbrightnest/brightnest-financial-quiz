...
export default function AdminDashboard() {
  ...
  const [isSubmittingAdminNote, setIsSubmittingAdminNote] = useState(false);
  ...
  const handleAdminCreateNote = async () => {
    if (!adminNoteContent.trim() || !crmSelectedLead) return;

    setIsSubmittingAdminNote(true);
    try {
      const leadEmail = crmSelectedLead.answers?.find((a: any) => a.value?.includes('@'))?.value;
      if (!leadEmail) {
        setIsSubmittingAdminNote(false);
        return;
      }
                          ...
                          <div className="bg-white rounded-lg border border-slate-200">
                            ...
                            <div className="border-b border-slate-200">
                              <div className="flex px-6">
                                {(['activity', 'notes', 'tasks'] as ('activity' | 'notes' | 'tasks')[]).map((tab) => (
                                  <button
                                    key={tab}
                                    onClick={() => setCrmLeadModalTab(tab)}
                            ...
                            {!crmActivities ? (
                              ...
                              {false && crmActivities && (crmActivities === null ? (
                                <div className="text-center py-8">
                                  <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-blue-600 mx-auto"></div>
                                  <p className="text-sm text-slate-600 mt-4">Loading activities...</p>
                                </div>
