      ...
      <div className="fixed left-0 top-20 w-72 h-[calc(100vh-5rem)] bg-white/80 backdrop-blur-sm border-r border-gray-200/60 z-40">
        <div className="p-6 h-full flex flex-col">
          ...
          <div className="mt-8 pt-6 border-t border-gray-200">
            <h3 className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-4">Progress Overview</h3>
            <div className="space-y-4">
              <div className="bg-gradient-to-r from-blue-50 to-blue-100 rounded-xl p-4">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-sm font-medium text-blue-900">Questions</span>
                  <span className="text-lg font-bold text-blue-900">{questions.length}</span>
                </div>
                <div className="w-full bg-blue-200 rounded-full h-2">
                  <div className="bg-blue-600 h-2 rounded-full transition-all duration-300" style={{width: `${Math.min((questions.length / 10) * 100, 100)}%`}}></div>
                </div>
              </div>
              
              <div className="bg-gradient-to-r from-emerald-50 to-emerald-100 rounded-xl p-4">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-sm font-medium text-emerald-900">Articles</span>
                  <span className="text-lg font-bold text-emerald-900">{articles.length}</span>
                </div>
                <div className="w-full bg-emerald-200 rounded-full h-2">
                  <div className="bg-emerald-600 h-2 rounded-full transition-all duration-300" style={{width: `${Math.min((articles.length / 5) * 100, 100)}%`}}></div>
                </div>
              </div>
              
              <div className="bg-gradient-to-r from-purple-50 to-purple-100 rounded-xl p-4">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-sm font-medium text-purple-900">Loading Screens</span>
                  <span className="text-lg font-bold text-purple-900">{loadingScreens.filter(ls => ls.triggerQuestionId).length}</span>
                </div>
                <div className="w-full bg-purple-200 rounded-full h-2">
                  <div className="bg-purple-600 h-2 rounded-full transition-all duration-300" style={{width: `${Math.min((loadingScreens.filter(ls => ls.triggerQuestionId).length / 3) * 100, 100)}%`}}></div>
                </div>
              </div>
            </div>
          </div>
          ...
          {questions.map((question, index) => {
            ...
            <div key={question.id} className="space-y-6">
              ...
              <div
                ...
                className={`group relative bg-white rounded-2xl shadow-sm border transition-all duration-300 hover:shadow-lg ${
