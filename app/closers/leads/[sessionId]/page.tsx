                        ...
                        <div key={activity.id} className="relative flex items-start space-x-4">
                          ...
                          <div className="flex-1 bg-slate-50 rounded-lg p-4 border border-slate-200">
                            <p className="text-sm font-semibold text-slate-900">
                              ...
                              {`Activity: ${activity.type.replace(/_/g, ' ')}`}
                            </p>
                            ...
                            {(activity.type === 'outcome_updated' || activity.type === 'outcome_marked' || activity.type === 'deal_closed') && (
                              <div className="mt-3">
                                <button onClick={() => setExpandedActivity(expandedActivity === activity.id ? null : activity.id)} className="text-xs text-blue-600 hover:text-blue-700 font-medium">
                                  {expandedActivity === activity.id ? 'Hide details' : 'View details'}
                                </button>
                                {expandedActivity === activity.id && (
                                  <div className="mt-2 text-sm text-slate-700">
                                    {activity.details?.outcome && <p>Outcome: {activity.details.outcome}</p>}
                                    {activity.details?.saleValue && <p>Sale Value: ${activity.details.saleValue}</p>}
                                    {activity.details?.notes && <p>Notes: {activity.details.notes}</p>}
                                    {activity.details?.recordingLink && <p>Recording: <a href={activity.details.recordingLink} className="text-blue-500" target="_blank" rel="noreferrer">Link</a></p>}
                                  </div>
                                )}
                              </div>
                            )}
