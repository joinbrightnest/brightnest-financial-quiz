...
export async function POST() {
  try {
    ...
    // Create Health Finance questions (same questions)
    for (const question of financialProfileQuestions) {
      await prisma.quizQuestion.create({
        data: {
    ...
    // Create Marriage Finance questions (same questions)
    for (const question of financialProfileQuestions) {
      await prisma.quizQuestion.create({
        data: {
    ...
    return NextResponse.json({ 
      success: true, 
      message: "Database seeded successfully with 3 quiz types (30 questions total)" 
    });
  } catch (error) {
